...
// bool change_password(int clientsocket);
bool logout(int clientsocket);
bool manage_userrole(int clientsocket);
...
struct usercredentials admincredentials = {"tanvi", "33"};
...
*/
                ...
                case 3:
                if(manage_userrole(clientsocket))
                {
...
bool modify_employeedetails(int clientsocket)
{
    ...
    while (read(dbfile, &emprecord, sizeof(struct Employee)) > 0)
    {
        ...
        if (strCompareResult==0)          //changed
        {
            ...
            send(clientsocket, "Enter updated employee password: ", strlen("Enter updated employee password: "), 0);
...
printf("inside roles\n");
 char empid1[100] = "";
    send(clientsocket, "Enter the employee ID to update: ", strlen("Enter the employee ID to update: "), 0);
    ssize_t bytesread = 0;
    bytesread = recv(clientsocket, empid1, sizeof(empid1) - 1, 0);
    printf("bytesread : %zd\n", bytesread);
...
struct Employee emprecord1;
    bool found = false;
    printf("%s is emp id received\n", empid1);
    while (read(dbfile, &emprecord1, sizeof(struct Employee)) > 0)
    {
        printf("Debug: Reading Employee Record: \n");
        printf("Employee ID: %s\n", emprecord1.employeeID);
        printf("Name: %s\n", emprecord1.name);
        int strCompareResult=strcmp(emprecord1.employeeID, empid1);
        printf("%d is the comparison result\n", strCompareResult);
        if (strCompareResult==0)          //changed
        {
            found = true;
            ...
            send(clientsocket, "Enter updated employee role: \n", strlen("Enter updated employee role: \n"), 0);
            bytesread = recv(clientsocket, emprecord1.role, sizeof(emprecord1.role) - 1, 0);
            if (bytesread <= 0)
            {
                send(clientsocket, "Error in receiving updates employee role", strlen("Error in receiving updates employee role"), 0);
                return false;
            }
            emprecord1.role[bytesread] = '\0';

            // Move the file pointer back to the beginning of the record and update it
            lseek(dbfile, (-1)*sizeof(struct Employee), SEEK_CUR);
            write(dbfile, &emprecord1, sizeof(struct Employee));
            break;
        }
    }
